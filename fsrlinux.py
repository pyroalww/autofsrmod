...
if not os.access(os.getcwd(), os.W_OK):
    messagebox.showerror("Error", "Insufficient permissions to write to the current directory.")
    sys.exit()
required_files = [
...
for file in required_files:
    if not os.path.exists(file):
        messagebox.showerror("Error", f"Required file '{file}' not found. Please check your installation.")
        sys.exit()
version_mapping = {
...
def browse_folder():
    initial_path = "/home/deck/.steam/steam/steamapps/compatdata"  # Update the path according to Steam Deck's file structure
...
games_combobox.grid(row=1, column=0, padx=10, pady=10, columnspan=3, sticky="nsew")

game_folder_label = ttk.Label(root, text="Game Folder:")
game_folder_label.grid(row=2, column=0, padx=10, pady=10, sticky="e")

game_folder_entry = ttk.Entry(root, font=('Helvetica', 16), width=50)
game_folder_entry.grid(row=2, column=1, padx=10, pady=10, sticky="w")

browse_button = ttk.Button(root, text="Browse", command=browse_folder, style="Large.TButton")
browse_button.grid(row=2, column=2, padx=10, pady=10, sticky="nsew")

activation_button = ttk.Button(root, text="Install AMD FSR3", command=enable_amd_fsr_simulation, style="Large.TButton")
activation_button.grid(row=3, column=0, columnspan=3, padx=10, pady=10, sticky="nsew")

remove_fsr_mod_button = ttk.Button(root, text="Remove FSR Mod", command=remove_fsr_mod_button, style="Large.TButton")
remove_fsr_mod_button.grid(row=4, column=0, columnspan=3, padx=10, pady=10, sticky="nsew")

progress_var = tk.IntVar()
progress_label = ttk.Label(root, text="")
progress_label.grid(row=5, column=0, padx=10, pady=10, columnspan=3)

progress_bar = ttk.Progressbar(root, mode="determinate", length=300)
progress_bar.grid(row=6, column=0, padx=10, pady=10, columnspan=3)

info_text = tk.StringVar()
info_label = ttk.Label(root, textvariable=info_text, font=("Helvetica", 14), foreground="blue")
info_label.grid(row=7, column=0, padx=10, pady=10, columnspan=3)

info1 = tk.StringVar()
info_label1 = ttk.Label(root, textvariable=info1, font=("Helvetica", 14), foreground="blue")
info_label1.grid(row=8, column=0, padx=12, pady=11, columnspan=4)

# Use the grid_rowconfigure and grid_columnconfigure to make the UI resizable
for i in range(9):
